...
export default function ForgotPasswordPage() {
  ...
  const handleRequestReset = async (e) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to request reset. Connection error.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 p-8">
        ...
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Reset Password</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Enter your registered email address to receive a recovery code.</p>
        ...
        {!otpStep ? (
          <form onSubmit={handleRequestReset} className="space-y-4">
            ...
            <button disabled={loading} type="submit" className="w-full mt-6 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-4 rounded-xl transition-colors flex items-center justify-center gap-2 cursor-pointer">
              {loading ? 'Sending code...' : 'Send Recovery Code'} <ArrowRight className="w-4 h-4" />
            </button>
          </form>
        ) : (
          <form onSubmit={handleResetPassword} className="space-y-4">
            <div className="p-4 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-xl flex items-start gap-3 mb-4">
              ...
              <p className="text-sm text-indigo-900 dark:text-indigo-200 font-medium leading-relaxed">
                We've sent a 6-digit recovery code to your email.
              </p>
            </div>
            
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1.5">Recovery Code</label>
              <input 
                type="text" 
                required
                maxLength={6}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1.5">New Password</label>
              <div className="relative">
        ...
        <div className="mt-8 text-center text-sm font-medium text-slate-500 dark:text-slate-400">
          Remember your password? <Link to="/login" className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 font-bold ml-1">Sign in</Link>
        </div>
      </div>
    </div>
  );
